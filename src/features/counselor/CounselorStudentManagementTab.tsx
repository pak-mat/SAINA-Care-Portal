...
export default function CounselorStudentManagementTab({ onStartChat }: { onStartChat?: (id: string) => void }) {
 ...
 setDrawerOpen(true);
  ...
  const handleUpdateStudent = async (data: Partial<User>) => {
    ...
    // Extract non-schema fields into preferences to avoid Supabase schema cache errors
    const { accountStatus, guardianName, emergencyContact, riskLevel, studentId, ...rest } = data;
    
    const prefs = {
      ...(editingStudent.preferences || {}),
    ...
    updateUser.mutate({ id: editingStudent.id, data: updatePayload }, {
      onSuccess: () => {
        setDrawerOpen(false);
      }
    });
 ...
 <div className="max-w-[1400px] mx-auto bg-white/50 dark:bg-zinc-900/50 backdrop-blur-xl border border-white/20 dark:border-zinc-800/50 rounded-xl shadow-xl overflow-hidden">
 ...
 <div className="w-14 h-14 rounded-lg bg-emerald-500/20 backdrop-blur-sm flex items-center justify-center border border-emerald-400/30 shadow-lg">
