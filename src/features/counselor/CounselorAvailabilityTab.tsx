    ...
    <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-10}} className="max-w-5xl mx-auto pb-8">
      <div className="mb-8 overflow-hidden bg-gradient-to-r from-emerald-900 to-slate-900 dark:from-emerald-950 dark:to-black rounded-[1.75rem] p-8 sm:p-10 relative shadow-lg">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          ...
          <button 
            onClick={handleSave}
            className="hidden sm:flex bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm px-6 py-3 rounded-xl items-center justify-center gap-2 transition-all shadow-xl hover:shadow-emerald-500/20 transform hover:-translate-y-0.5"
          >
            ...
            >
              <div className={`p-5 border-b border-white/50 dark:border-zinc-800/50 flex items-center justify-between transition-colors ${isWorking ? 'bg-emerald-50/50 dark:bg-emerald-900/10' : 'bg-transparent'}`}>
                ...
                >
                  ...
                  <span
                    aria-hidden="true"
                    className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow-sm ring-0 transition duration-300 ease-in-out ${isWorking ? 'translate-x-5' : 'translate-x-0'}`}
                  />
                </button>
              </div>
      ...
      <div className="mt-8 flex sm:hidden">
        <button 
          onClick={handleSave}
          className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm px-6 py-4 rounded-xl flex justify-center items-center gap-2 transition-all shadow-lg transform active:scale-95"
        >
          {saved ? <><CheckCircle size={18} /> Saved</> : <><Save size={18} /> Update Schedule</>}
        </button>
      </div>
