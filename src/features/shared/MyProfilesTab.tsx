...
export default function MyProfilesTab({ onTabChange }) {
  ...
  const { groups: myGroupChats, createGroup } = useGroupChats(user?.id);
  ...
  // Stubs for followers/following (if not implemented in new schema, default to empty to not break UI)
  const followingIds = [];
  const followersIds = [];
  ...
  // Chat Data Hooks
  const { messages: dmMessages, sendMessage: sendDM } = useDirectMessages(user?.id, activeDMPartnerId);
  const { messages: groupMessages, sendMessage: sendGrpMsg } = useGroupMessages(activeGroupId);
  const currentChatMessages = activeChatType === 'direct' ? dmMessages : groupMessages;
  ...
  const handleToggleFollow = (targetId, e) => {
    ...
    // Stub: Following is handled by Friends table now
  };
  ...
  const handleSendChatMessage = (e) => {
    ...
    if (activeChatType === 'direct' && activeDMPartnerId) {
      sendDM.mutate(chatInputText);
      playBeep(1100, 'sine', 0.06);
    } else if (activeChatType === 'group' && activeGroupId) {
      sendGrpMsg.mutate({ senderId: user.id, senderName: user.name, text: chatInputText });
  ...
  const handleCreateGroupSubmit = (e) => {
    ...
    createGroup.mutate({ name: newGroupName, members: newGroupMembers }, {
      onSuccess: (newG) => {
        setNewGroupName('');
        setNewGroupMembers([]);
  ...
  return (
    ...
    >
      ...
      <div className={`mb-8 overflow-hidden bg-gradient-to-r ${isStudent ? 'from-emerald-900 to-slate-900 dark:from-emerald-950 dark:to-black' : 'from-blue-900 to-slate-900 dark:from-blue-950 dark:to-black'} rounded-[1.75rem] p-8 sm:p-10 relative shadow-lg shrink-0 mx-2 mt-2 sm:mx-4 sm:mt-4`}>
        <div className={`absolute top-0 right-0 w-64 h-64 bg-${themeColor}-500 opacity-20 rounded-full blur-3xl transform translate-x-1/2 -translate-y-1/4 pointer-events-none`}></div>
          ...
          <div className="flex bg-black/20 backdrop-blur-md p-1.5 rounded-xl self-start border border-white/10 shadow-inner flex-wrap gap-1 md:flex-nowrap">
            ...
            >
              ...
              {requestsReceivedIds.length > 0 && (
                <span className="bg-rose-500 text-white font-mono text-[10px] px-2 py-0.5 rounded-full animate-bounce ml-1 shadow-sm">
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredDirectory.map((co) => {
                ...
                const { isOnline: isCardUserOnline } = usePresence(co.id);
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="px-5 pb-1 relative flex flex-col items-center">
                        ...
                        <div className={`relative w-16 h-16 rounded-full -mt-8 border-3 border-white dark:border-zinc-900 flex items-center justify-center font-black text-lg shadow-sm transition-transform group-hover:scale-105 duration-300 ${getAvatarClass(co.avatarColor)}`}>
                          {getInitials(co.name)}
                          {isCardUserOnline && <div className="absolute top-0 right-0 w-4 h-4 bg-emerald-500 border-2 border-white dark:border-zinc-900 rounded-full" />}
                        </div>
