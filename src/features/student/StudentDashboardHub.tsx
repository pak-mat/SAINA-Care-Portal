...
function DailyWellnessCheckin({ userId }: { userId: string }) {
 ...
 className="bg-white dark:bg-zinc-900 w-full max-w-md rounded-xl shadow-2xl overflow-hidden relative z-10 border border-slate-100 dark:border-zinc-800"
 >
 ...
 className={`flex flex-col items-center gap-2 p-3 rounded-lg transition-all ${mood === m.value ? 'bg-emerald-50 dark:bg-emerald-900/30 scale-110 shadow-sm' : 'hover:bg-slate-50 dark:hover:bg-zinc-800 grayscale hover:grayscale-0 opacity-60 hover:opacity-100'}`}
 >
...
function HomeTab({ user, requests, notifications, onNavigate }) {
 ...
 >
 <div className="absolute top-0 right-0 w-64 h-64 bg-emerald-500 opacity-10 rounded-full blur-3xl translate-x-1/2 -translate-y-1/4" />
 <div className="absolute bottom-0 left-0 w-48 h-48 bg-teal-500 opacity-10 rounded-full blur-2xl -translate-x-1/4 translate-y-1/4" />
 {/* Dot grid */}
 ...
 className="bg-white/5 rounded-lg p-5 backdrop-blur-md border border-white/10 min-w-[110px] flex flex-col items-center text-center shadow-xl"
 >
 ...
 <ChevronRight size={14} className={`${action.iconColor} mt-auto group-hover:translate-x-1 `} />
 </motion.button>
 );
 })}
 </div>
 ...
 <div className="w-14 h-14 rounded-lg bg-slate-100 dark:bg-zinc-800 flex items-center justify-center">
